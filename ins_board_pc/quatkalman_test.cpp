...
QuatKalmanTest::QuatKalmanTest(const FilterParams & par)
...
QuatKalmanTest::~QuatKalmanTest()
{

}

void QuatKalmanTest::accumulate(const FilterInput & z)
{
    AbstractOrientationFilter::accumulate(z);
    AbstractPositionFilter::accumulate(z);
}

void QuatKalmanTest::reset()
{
    AbstractOrientationFilter::reset();
    AbstractPositionFilter::reset();
}

void QuatKalmanTest::initialize(const FilterInput & z)
{
...
void QuatKalmanTest::step(const FilterInput & z)
{
    if(is_initialized())
    {
        update(z);
...
void QuatKalmanTest::update(const FilterInput & z)
{
...
void QuatKalmanTest::normalize_state()
{
    ublas::project(x, ublas::range(0, 4)) = qutils::quat_normalize(get_orientation_quaternion());
}

NumMatrix QuatKalmanTest::create_transition_mtx(const FilterInput & z)
{
    /* useful constants */
    double dt = z.dt;
...
NumMatrix QuatKalmanTest::create_proc_noise_cov_mtx(double dt)
{
    /* useful constants */
    double dt_sq = dt * dt;
    double dt_2 = dt / 2;
...
NumMatrix QuatKalmanTest::create_meas_noise_cov_mtx(double lat, double lon, double magn_mag)
{
...
NumMatrix QuatKalmanTest::create_meas_proj_mtx(double lat, double lon, double alt, QDate day, const NumVector & v)
{
    // 1
...
void QuatKalmanTest::calculate_geodetic(const NumVector & position,
                                          double & lat, double & lon, double & alt)
{
    WrapperWMM::instance().cartesian_to_geodetic(position, lat, lon, alt);
}

void QuatKalmanTest::calculate_accelerometer(const NumVector & orientation_quat, const NumVector & acceleration,
                             double lat, double lon, double alt,
...
void QuatKalmanTest::calculate_magnetometer(const NumVector & orientation_quat,
...
void QuatKalmanTest::calculate_velocity(const NumVector & velocity, double & vel)
{
    vel = norm_2(velocity);
}

NumVector QuatKalmanTest::get_orientation_quaternion()
{
    return ublas::project(x, ublas::range(0, 4));
}

NumVector QuatKalmanTest::get_gyro_bias()
{
    return ublas::project(x, ublas::range(4, 7));
}

NumVector QuatKalmanTest::get_position()
{
    return ublas::project(x, ublas::range(7, 10));
}

NumVector QuatKalmanTest::get_velocity()
{
    return ublas::project(x, ublas::range(10, 13));
}

NumVector QuatKalmanTest::get_acceleration()
{
    return ublas::project(x, ublas::range(13, 16));
}

void QuatKalmanTest::get_rpy(double & roll, double & pitch, double & yaw)
{
    qutils::quat_to_rpy(get_orientation_quaternion(), roll, pitch, yaw);
}

void QuatKalmanTest::get_geodetic(double & lat, double & lon, double & alt)
{
    calculate_geodetic(get_position(), lat, lon, alt);
}

void QuatKalmanTest::set_proc_gyro_std(double std)
{
    params.proc_params.gyro_std = std;
}

void QuatKalmanTest::set_proc_gyro_bias_std(double std)
{
    params.proc_params.gyro_bias_std = std;
}

void QuatKalmanTest::set_proc_accel_std(double std)
{
    params.proc_params.accel_std = std;
}

void QuatKalmanTest::set_meas_accel_std(double std)
{
    params.meas_params.accel_std = std;
}

void QuatKalmanTest::set_meas_magn_std(double std)
{
    params.meas_params.magn_std = std;
}

void QuatKalmanTest::set_meas_pos_std(double std)
{
    params.meas_params.gps_cep = std;
}

void QuatKalmanTest::set_meas_vel_std(double std)
{
    params.meas_params.gps_vel_abs_std = std;
}

void QuatKalmanTest::set_init_quat_std(double std)
{
    params.init_params.quat_std = std;
}

void QuatKalmanTest::set_init_bias_std(double std)
{
    params.init_params.bias_std = std;
}

void QuatKalmanTest::set_init_pos_std(double std)
{
    params.init_params.pos_std = std;
}

void QuatKalmanTest::set_init_vel_std(double std)
{
    params.init_params.vel_std = std;
}

void QuatKalmanTest::set_init_accel_std(double std)
{
